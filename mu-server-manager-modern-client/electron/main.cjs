...
mainWindow.webContents.on("did-fail-load", (event, errorCode, errorDescription) => {
  ...
  // Cleanup adicional: força parada de animações CSS quando a janela está sendo fechada
  mainWindow.on("close", () => {
    try {
      // Envia comando para o frontend parar todas as animações
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.executeJavaScript(`
          ...
          // (não há API nativa, mas o React já limpa os seus)
        `).catch(() => {
          // Ignora erros se a janela já foi destruída
        });
      }
    } catch (err) {
